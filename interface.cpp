...
int main(int argc, char *argv[]){
  ...
  printf("\n\nlaunching GPU...\n\n");
	GPU_Palette P1;
	P1 = openPalette(1024, 1024); // width, height
...
void myDraw(GPU_Palette* P1, CPUAnimBitmap* A1, void* theAudio){
	...
	for (int i = 0; i < numFrames; i++){
		...
		leftAmp = getAmp(audio->pos, frameStart, frameEnd, 0)/(numSampsPerFrame * 1.0);
		rightAmp = getAmp(audio->pos, frameStart, frameEnd, 2)/(numSampsPerFrame * 1.0);
