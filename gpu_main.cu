...
/******************************************************************************/
int updatePalette(GPU_Palette* P, float leftAmp, float rightAmp)
{

  // drop your reds, drop your greens and blues :)
  updateReds <<< P->gBlocks, P->gThreads >>> (P->red, P->rand, P->num_pixels);
  updateGreens <<< P->gBlocks, P->gThreads >>> (P->green, P->num_pixels, leftAmp);
	updateBlues <<< P->gBlocks, P->gThreads >>> (P->blue, P->num_pixels, rightAmp);
...
/******************************************************************************/
__global__ void updateGreens(float* green, unsigned long numPixels, float amp)
{
  int x = threadIdx.x + (blockIdx.x * blockDim.x);
  int y = threadIdx.y + (blockIdx.y * blockDim.y);
  int vecIdx = x + (y * blockDim.x * gridDim.x);

  if(vecIdx < numPixels) green[vecIdx] = amp;
}

/******************************************************************************/
__global__ void updateBlues(float* blue, unsigned long numPixels, float amp)
{
  ...
  if(vecIdx < numPixels) blue[vecIdx] = amp;
}
